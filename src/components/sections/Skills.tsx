...
export const Skills: React.FC = () => {
  return (
    <section id="skills" className="relative py-24 z-10 overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-[var(--bg-page)] via-[#D0D6F5]/60 to-[var(--bg-page)] opacity-80" />
          ...
          {SKILLS_DATA.map((category, categoryIndex) => {
            ...
            return (
              <motion.div 
                ...
                className="glass-card p-6 sm:p-8 rounded-3xl border border-[var(--glass-border)]"
              >
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-3 bg-[var(--purple-soft)]/10 rounded-xl text-[var(--purple-primary)]">
                    {IconNode}
                  </div>
                  <h3 className="text-xl font-bold font-heading text-[var(--text-deep)]">{category.category}</h3>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {category.skills.map((skill, skillIndex) => (
                    <motion.div
                      ...
                      className="group flex items-center gap-2 px-4 py-2.5 bg-white/60 hover:bg-white border border-white/60 hover:border-[var(--purple-soft)]/50 rounded-xl transition-all cursor-default shadow-sm"
                    >
